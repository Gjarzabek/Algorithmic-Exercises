...
public:
    ...
    int minPathSum(std::vector<std::vector<int>>& grid) {
      std::vector<std::vector<int>> sums(grid.size(), std::vector<int>(grid[0].size(), -1));
      std::stack<std::pair<int, int>> points;
      std::pair<int, int> && curr_pos = std::make_pair(0, 0);
      points.push(curr_pos);
      sums[0][0] = grid[0][0];
      while (!points.empty()) {
        int sum = sums[curr_pos.first][curr_pos.second];
        if (curr_pos.first > 0) {
          if (sums[curr_pos.first-1][curr_pos.second] < 0)
          {
            sums[curr_pos.first-1][curr_pos.second] = sum + grid[curr_pos.first-1][curr_pos.second];
            points.push(std::make_pair(curr_pos.first-1, curr_pos.second));
          }
          else if (sum + grid[curr_pos.first-1][curr_pos.second] < sums[curr_pos.first-1][curr_pos.second]) {
            sums[curr_pos.first-1][curr_pos.second] = sum + grid[curr_pos.first-1][curr_pos.second];
            points.push(std::make_pair(curr_pos.first-1, curr_pos.second));
          }
        ...
        if (curr_pos.second > 0) {
          if (sums[curr_pos.first][curr_pos.second-1] < 0 ||
            sum + grid[curr_pos.first][curr_pos.second-1] < sums[curr_pos.first][curr_pos.second-1])
          {
            sums[curr_pos.first][curr_pos.second-1] = sum + grid[curr_pos.first][curr_pos.second-1];
            points.push(std::make_pair(curr_pos.first, curr_pos.second-1));
          }
        }
        if (curr_pos.second < grid[0].size() - 1) {
          if (sums[curr_pos.first][curr_pos.second+1] < 0 ||
            sum + grid[curr_pos.first][curr_pos.second+1] < sums[curr_pos.first][curr_pos.second+1])
          {
            sums[curr_pos.first][curr_pos.second+1] = sum + grid[curr_pos.first][curr_pos.second+1];
        ...
        if (curr_pos.first < grid.size() - 1) {
          if (sums[curr_pos.first+1][curr_pos.second] < 0 ||
            sum + grid[curr_pos.first+1][curr_pos.second] < sums[curr_pos.first+1][curr_pos.second])
          {
            sums[curr_pos.first+1][curr_pos.second] = sum + grid[curr_pos.first+1][curr_pos.second];
            points.push(std::make_pair(curr_pos.first+1, curr_pos.second));
          }
